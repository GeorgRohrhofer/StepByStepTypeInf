...
function unifyTypes(
    ...
    return new Failure(`Type mismatch: ${formatInferredType(aa)} vs ${formatInferredType(bb)}`);
}
...
export function collectBetaReductionSteps(start: Term): { term: Term; steps: { from: Term; to: Term }[] } {
...
function getFreeVariables(term: Term, bound: Set<string> = new Set()): Set<string> {
    if (term.kind === "var") {
